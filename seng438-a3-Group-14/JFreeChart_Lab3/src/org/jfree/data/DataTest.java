...
public class DataTest extends DataUtilities {
 ...
 // ##########################Adding new tests################################
    ...
    public void testCalculateColumnTotalWithNegativeRowCount() {
        ...
        mockingContext.checking(new Expectations() {{
            ...
            exactly(1000).of(data).getValue(with(any(Integer.class)), with(equal(column))); will(returnValue(1.0)); // Allow exactly 1000 invocations
        }});
 ...
 @Test
    public void testCalculateColumnTotalValidRowsCoverage() {
        ...
        final Values2D data = mockingContext.mock(Values2D.class);
        final int column = 0;
        final int[] validRows = {0, 1}; // Assuming these are the valid row indices

        // Assume row count is more than the highest index in validRows
 ...
 @Test
    public void testCalculateRowTotalCoverSecondLoop() {
        ...
        // Assuming the columnCount can be less than 0 to satisfy the loop condition
        mockingContext.checking(new Expectations() {{
            oneOf(data).getColumnCount(); will(returnValue(-1)); // force the loop to execute
            exactly(1000).of(data).getValue(with(any(Integer.class)), with(equal(row))); will(returnValue(1.0));
